...
dag_rabbitmq = DAG(
    ...
    description="rabbitmq image processing DAG",
    schedule_interval="* * * 1 *",
    catchup=False,
    tags=["image, enhance"],
)
...
# Creating first task
start_task = DummyOperator(task_id="start_task", dag=dag_rabbitmq)

producer_task = PythonOperator(
    task_id="load_db_queue_task", python_callable=load_db_queue, dag=dag_rabbitmq
)

# Creating last task
end_task = DummyOperator(task_id="end_task", dag=dag_rabbitmq)
